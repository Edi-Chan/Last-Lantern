...
func test_water_generation_keeps_water_not_lava() -> void:
	...
	assert_true(types.contains("LAVA = 2"))
	...
	var lava := _read("res://scripts/liquid/lava_generation.gd")
	assert_true(lava.contains("LiquidTypes.Type.LAVA"))
	assert_true(lava.contains("is_fire_region"))
	assert_true(lava.contains("LiquidBasin.fill"))
	assert_true(water.contains("LiquidBasin.fill"))
	assert_true(water.contains("_pick_cave_water_floor"))
