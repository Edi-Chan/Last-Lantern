...
func test_scripts_and_scenes_exist() -> void:
	...
	assert_true(ResourceLoader.exists("res://scripts/ui/game_cursor.gd"))
	...
	assert_true(zombie.contains("func get_display_name"))
	...
	assert_true(hud.contains("_ensure_game_cursor"))
	...
	assert_true(mgr.contains("is_inspectable"))
	assert_true(mgr.contains("get_aimed_target"))
	...
	assert_true(bar.contains("set_identity"))
	assert_true(bar.contains("ANIMAL"))
...
func test_hover_identity_shows_name_and_hp() -> void:
	var bar := EnemyHealthBar.new()
	bar._ready()
	bar.bind_health(12.0, 40.0, true)
	bar.set_identity("Zombie", true)
	assert_true(bar.show_identity)
	assert_eq(bar._name_label.text, "Zombie")
	assert_eq(bar._label.text, "12 / 40")
	assert_true(bar._name_label.visible)
	assert_true(bar._label.visible)
	bar.set_identity("Zombie", false)
	assert_false(bar._name_label.visible)
	bar.free()


func test_inspectable_includes_animals_and_not_player() -> void:
	var src := _read("res://scripts/ui/enemy_health_bar_manager.gd")
	assert_true(src.contains("func is_inspectable"))
	assert_true(src.contains("is_in_group(\"animals\")"))
	assert_true(src.contains("is_in_group(\"enemies\")"))
	assert_true(src.contains("is_in_group(\"player\")"))
	assert_true(src.contains("func get_aimed_target"))
	assert_true(src.contains("is_in_group(\"animals\")"))
	assert_true(_read("res://scripts/animals/animal_base.gd").contains("func get_display_name"))
	assert_true(_read("res://scripts/enemies/zombie.gd").contains("func get_display_name"))


func test_pixel_cursor_assets_and_wiring() -> void:
	assert_true(FileAccess.file_exists("res://assets/ui/cursor/pointer.png"))
	assert_true(FileAccess.file_exists("res://assets/ui/cursor/crosshair.png"))
	assert_true(FileAccess.file_exists("res://assets/ui/cursor/target.png"))
	assert_true(ResourceLoader.exists("res://scripts/ui/game_cursor.gd"))
	var cursor := _read("res://scripts/ui/game_cursor.gd")
	assert_true(cursor.contains("MOUSE_MODE_HIDDEN"))
	assert_true(cursor.contains("MOUSE_MODE_VISIBLE"))
	assert_true(cursor.contains("Kind.CROSSHAIR"))
	assert_true(cursor.contains("Kind.TARGET"))
	assert_true(cursor.contains("inventory_ui"))
	assert_true(cursor.contains("set_custom_mouse_cursor"))
	assert_true(cursor.contains("PROCESS_MODE_ALWAYS"))
	assert_true(_read("res://project.godot").contains("PixelCursor="))
	assert_true(_read("res://scripts/animals/animal_base.gd").contains("func get_display_name"))
	assert_true(cursor.contains("func wants_pointer"))
	assert_true(cursor.contains("Kind.POINTER"))
